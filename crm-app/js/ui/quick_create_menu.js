...
const ensureTaskModalElements = () => { if (typeof document === 'undefined') return null; if (taskModalState.overlay && taskModalState.overlay.isConnected !== false) return taskModalState.overlay; const overlay = document.createElement('div'); overlay.id = 'qc-task-modal'; overlay.dataset.role = 'qc-task-modal'; overlay.style.cssText = 'position:fixed;inset:0;z-index:10060;display:flex;align-items:center;justify-content:center;padding:16px;background:rgba(15,23,42,0.45);'; overlay.hidden = true; overlay.style.visibility = 'hidden'; overlay.style.pointerEvents = 'none'; overlay.setAttribute('role', 'dialog'); overlay.setAttribute('aria-modal', 'true'); overlay.innerHTML = TASK_MODAL_HTML; const form = overlay.querySelector('form[data-role="form"]'); const typeSelect = overlay.querySelector('select[name="linkedType"]'); const entitySelect = overlay.querySelector('select[name="linkedId"]'); const dueInput = overlay.querySelector('input[name="due"]'); const noteInput = overlay.querySelector('textarea[name="note"]'); const status = overlay.querySelector('[data-role="status"]'); const closeBtn = overlay.querySelector('button[data-role="close"]'); const cancelBtn = overlay.querySelector('button[data-role="cancel"]'); const saveBtn = overlay.querySelector('button[data-role="save"]'); if (!form || !typeSelect || !entitySelect || !dueInput || !noteInput || !status || !closeBtn || !cancelBtn || !saveBtn) return null; closeBtn.addEventListener('click', (event) => { event.preventDefault(); closeTaskModal(); }); cancelBtn.addEventListener('click', (event) => { event.preventDefault(); closeTaskModal(); }); overlay.addEventListener('click', (event) => { if (event.target === overlay) closeTaskModal(); }); typeSelect.addEventListener('change', () => refreshTaskEntityOptions(typeSelect.value)); form.addEventListener('submit', (event) => handleTaskModalSubmit(event)); Object.assign(taskModalState, { overlay, form, typeSelect, entitySelect, dueInput, noteInput, status, saveBtn }); document.body.appendChild(overlay); return overlay; };
...
const refreshTaskEntityOptions = async (kind) => { const select = taskModalState.entitySelect; if (!select) return; const saveBtn = taskModalState.saveBtn; taskModalState.loadingToken += 1; const token = taskModalState.loadingToken; select.innerHTML = '<option value="">Loading…</option>'; select.disabled = true; if (saveBtn) saveBtn.disabled = true; const options = await fetchTaskEntityOptions(kind); if (token !== taskModalState.loadingToken) return; select.innerHTML = ''; if (!options.length) { const option = document.createElement('option'); option.value = ''; option.textContent = kind === 'partner' ? 'No partners available' : 'No contacts available'; select.appendChild(option); setTaskModalStatus('Add a record before creating a task.', 'error'); select.disabled = true; return; } options.forEach((opt) => { const option = document.createElement('option'); option.value = opt.value; option.textContent = opt.label; select.appendChild(option); }); select.disabled = false; if (saveBtn) saveBtn.disabled = false; select.value = options[0].value; setTaskModalStatus(''); };
const handleTaskModalSubmit = async (event) => { if (event && typeof event.preventDefault === 'function') event.preventDefault(); const { typeSelect, entitySelect, noteInput, dueInput, saveBtn } = taskModalState; if (!typeSelect || !entitySelect || !dueInput) return; if (taskModalState.busy) return; const kind = typeSelect.value === 'partner' ? 'partner' : 'contact'; const linkedId = String(entitySelect.value || '').trim(); if (!linkedId) { setTaskModalStatus(kind === 'partner' ? 'Select a partner.' : 'Select a contact.', 'error'); try { entitySelect.focus({ preventScroll: true }); } catch (_) { entitySelect.focus(); } return; } const due = String(dueInput.value || todayISODate() || '').trim(); const note = noteInput ? String(noteInput.value || '').trim() : ''; taskModalState.busy = true; if (saveBtn) saveBtn.disabled = true; setTaskModalStatus('Saving task…', 'info'); try { const mod = await import('../tasks/api.js'); const fn = mod && typeof mod.createMinimalTask === 'function' ? mod.createMinimalTask : mod && typeof mod.createTask === 'function' ? mod.createTask : mod && typeof mod.default === 'function' ? mod.default : null; if (typeof fn !== 'function') throw new Error('Task API unavailable'); const payload = { linkedType: kind, linkedId, due }; if (note) payload.note = note; const result = await fn(payload); if (result && result.status === 'ok' && result.task && result.task.id && typeof window !== 'undefined') window.__QC_TASK_ID__ = result.task.id; toastInfo('Task created'); setTaskModalStatus('Task saved.', 'success'); closeTaskModal(); } catch (err) { if (console && typeof console.warn === 'function') console.warn('[quick-create] task save failed', err); setTaskModalStatus('Unable to save task. Try again.', 'error'); toastWarn('Task could not be saved'); } finally { taskModalState.busy = false; if (saveBtn) saveBtn.disabled = false; } };
const openTaskQuickCreateModal = () => { const overlay = ensureTaskModalElements(); if (!overlay) { toastWarn('Task form unavailable'); return null; } const { typeSelect, dueInput, noteInput } = taskModalState; overlay.hidden = false; overlay.style.visibility = 'visible'; overlay.style.pointerEvents = 'auto'; overlay.setAttribute('aria-hidden', 'false'); taskModalState.busy = false; if (dueInput) { const today = todayISODate(); if (today) dueInput.value = today; } if (noteInput) noteInput.value = ''; setTaskModalStatus(''); refreshTaskEntityOptions(typeSelect ? typeSelect.value : 'contact'); if (noteInput && typeof noteInput.focus === 'function') { try { noteInput.focus({ preventScroll: true }); } catch (_) { noteInput.focus(); } } return overlay; };

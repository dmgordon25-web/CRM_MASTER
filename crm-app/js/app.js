...
(function () {
  ...
  (function installAppDataChangedHandler() {
    ...
    const handler = function (evt) {
      ...
      const hasScopedMutation = [normalizedDetailScope, normalizedPartialScope].some(scopeKey => (
        !!scopeKey && KNOWN_SCOPES.has(scopeKey)
      ));
      if (!selectionOnly && hasScopedMutation) {
        clearAllSelectionScopes();
      }
      if (partial) {
        const lanes = [];

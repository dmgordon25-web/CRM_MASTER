export const STR: Record<string, string> = {
  'general.close': 'Close',
  'general.save': 'Save',
...
export function legacyText(key: string, params?: Record<string, string | number>): string {
  return text(key, params);
}

if(typeof window !== 'undefined'){
  (window as any).STR = STR;
  (window as any).getUiString = text;
}

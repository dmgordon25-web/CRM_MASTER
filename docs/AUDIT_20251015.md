...
- Workbench renders read-only tables with selection mirroring, but lacks saved filters, action bar integration, or scoped bulk actions—currently a monitoring surface rather than an operational console.
...
## Workbench
- `pages/workbench.js` lazy-mounts `#view-workbench`, renders header + two tables (contacts, partners) with inline Refresh buttons.
- Data fetch: `fetchAllRows()` reads IndexedDB stores (`openDB`, `dbGetAll`), sorts by `updatedAt` desc. No pagination or filters; multi-thousand rows will freeze UI.
- Selection: tables use `[data-selection-scope]` and integrate with global Selection (toggle on row click). However, there’s no action bar integration; selecting rows does nothing except highlight.
- Self-test hook: `renderWorkbench` accepts `onRunSelfTest` (wired via dynamic import of `selftest.js` when Workbench nav triggered). UI disables button when handler missing.
- Styling: uses inline styles; no saved filters or quick create. Documented gap: no CSV export, no saved filter sets, no integration with pipeline actions.

...
async function performOpen(targetId, meta, isNew){
  ...
  try{
    ...
    if(queued){
      ...
      state.pending = performOpen(queued.contactId, queued.meta, queued.isNew);
      state.pending.catch(() => {});
    }
  }
}
...
export function openContactEditor(contactId, meta){
  ...
  if((statusBefore === 'opening' || statusBefore === 'closing') && state.currentId && targetId && state.currentId !== targetId){
    logDebug('open requested during transition for different contact', { currentId: state.currentId, targetId });
  }

  if(statusBefore === 'opening' || statusBefore === 'closing'){
    state.queue = { contactId: targetId, meta, isNew: false };
    return state.pending || Promise.resolve(null);
  }

  if(statusBefore === 'open'){
    if(state.currentId && targetId && state.currentId === targetId){
      logDebug('focus existing contact editor', { targetId });
      return state.pending || Promise.resolve(null);
    }
    state.queue = { contactId: targetId, meta, isNew: false };
    return state.pending || Promise.resolve(null);
...
export function openNewContactEditor(meta){
  ...
  if(statusBefore === 'opening' || statusBefore === 'closing' || statusBefore === 'open'){
    state.queue = { contactId: '', meta, isNew: true };
    return state.pending || Promise.resolve(null);
  }
...
export default { openContactEditor, openNewContactEditor };

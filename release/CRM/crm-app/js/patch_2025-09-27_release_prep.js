...
function runPatch(){
    ...
    window.runReleaseChecklist = async function(){
      ...
      }catch (err) { console.warn('[soft] release checklist failed', err); }
      finally{
        running = false;
        if(btn){
          btn.disabled = false;
...
export async function init(ctx){
  ...
  if(__wired){
    log('[patch_2025-09-27_release_prep.init] already wired');
...
ensureCRM();
window.CRM.modules['patch_2025-09-27_release_prep'] = window.CRM.modules['patch_2025-09-27_release_prep'] || {};
window.CRM.modules['patch_2025-09-27_release_prep'].init = init;

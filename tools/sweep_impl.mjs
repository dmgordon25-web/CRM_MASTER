...
async function detectUnreferenced() {
  ...
  const ignored = new Set(['boot/loader.js', 'boot/manifest.js', 'js/boot/loader.js', 'js/boot/manifest.js']);
  const unreferenced = [];
...
async function detectDuplicateListeners() {
  ...
  const findings = [];
  const listenerIgnore = new Set(['js/app.js', 'js/core/renderGuard.js']);
  for (const file of files) {
    ...
    if (matches && matches.length > 1) {
      const rel = relativeToApp(file);
      if (listenerIgnore.has(rel)) continue;
      findings.push({ file: rel, count: matches.length });
    }
  }
...
async function detectLegacyRoots() {
  ...
  for (const marker of legacyMarkers) {
    ...
    try {
      ...
      findings.push(stat.isDirectory() ? `${normalizeRel(marker)}/` : normalizeRel(marker));
    } catch {
...
function formatReport({ unreferencedModules, duplicateListeners, ellipsisMatches, legacyRoots, legacyCalendars }) {
  const lines = ['# Sweep Report', '', `Application root: ${path.basename(appRoot)}`, ''];
  const sections = [
...
async function main() {
  ...
  const hasFindings = hasStrictErrors || unreferencedModules.length > 0 || duplicateListeners.length > 0 || ellipsisMatches.length > 0;

...
The shared state bus also suppresses secondary refreshes from task status updates and app:data events by merging options before invoking `handleDashboardRefresh`, and the dashboard shell now reads mode changes directly from the bus instead of cycling local toggles.

...
async function main() {
  ...
  try {
    ...
    const actionCheck = await page.evaluate(() => new Promise((resolve) => {
      const pickAction = () => {
        const scope = document.getElementById('actionbar') || document;
        ...
        const mass = query('[data-act="emailMass"]');
        if (isEnabled(mass)) return mass;
        const together = query('[data-act="emailTogether"]');
        if (isEnabled(together)) return together;
        const clear = query('[data-act="clear"]');
        if (isEnabled(clear)) return clear;
        const anyAction = Array.from(scope.querySelectorAll('[data-act]')).find(isEnabled);
        return anyAction || null;
      };
      const btn = pickAction();
      if (!btn) {
        resolve({ ok: false, reason: 'action-missing' });
        return;
      }
      const beforeTick = Number(window.__LAST_TOAST_TICK__ || 0);
      const beforeMsg = typeof window.__LAST_TOAST__ === 'string' ? window.__LAST_TOAST__ : '';
      const hostSelector = '[data-toast-host="true"],[data-ui="toast"],[data-toast="true"],[data-role="toast"]';
      const readHostText = () => {
      ...
      const hostBefore = readHostText();
      let settled = false;
      const cleanup = () => {
        try { window.removeEventListener('ui:toast', onToast); }
        catch (_) {}
      };
      ...
      const onToast = (event) => {
        const tick = Number(window.__LAST_TOAST_TICK__ || beforeTick);
        const msg = String((event && event.detail && event.detail.msg) || window.__LAST_TOAST__ || readHostText() || '');
        finish({ ok: true, toastObserved: true, via: 'event', tick, msg, count: Number(window.__SEL_COUNT__ || 0) });
      };
      window.addEventListener('ui:toast', onToast, { once: true });
      btn.click();
      Promise.resolve().then(() => {
        if (settled) return;
        const tickNow = Number(window.__LAST_TOAST_TICK__ || 0);
        const hostNow = readHostText();
        const toastMsg = String(window.__LAST_TOAST__ || '');
        if (tickNow > beforeTick || toastMsg !== beforeMsg || hostNow !== hostBefore) {
          const msg = toastMsg || hostNow;
          finish({ ok: true, toastObserved: true, via: 'microtask', tick: tickNow, msg, count: Number(window.__SEL_COUNT__ || 0) });
          return;
        }
        requestAnimationFrame(() => {
          if (settled) return;
          const tickLater = Number(window.__LAST_TOAST_TICK__ || 0);
          const hostLater = readHostText();
          const toastLater = String(window.__LAST_TOAST__ || '');
          if (tickLater > beforeTick || toastLater !== beforeMsg || hostLater !== hostBefore) {
            const msg = toastLater || hostLater;
            finish({ ok: true, toastObserved: true, via: 'raf', tick: tickLater, msg, count: Number(window.__SEL_COUNT__ || 0) });
            return;
          }
          finish({ ok: false, reason: 'no-toast', toastObserved: false, count: Number(window.__SEL_COUNT__ || 0) });
        });
      });
    }));
    if (!actionCheck.ok) {

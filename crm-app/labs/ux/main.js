...
function setupDashboard() {
  ...
  const renderStatic = (message) => {
    host.classList.add('grid-stack-fallback');
    const banner = message
      ? `<div class="grid-fallback-banner">${escapeHtml(message)}</div>`
      : '';
    const cards = dashboardCards
      .map((card) => `<div class="grid-fallback-item">${card.content}</div>`)
      .join('');
    host.innerHTML = `${banner}${cards}`;
  };

  if (!GridStack || typeof GridStack.init !== 'function') {
    renderStatic('Interactive grid is unavailable — displaying static cards instead.');
    return;
  }

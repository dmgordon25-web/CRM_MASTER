...
function runPatch() {
  ...
  function onDataChanged(evt) {
    // Quarantine: Only log if diagnostics are actively running/viewed
    if (!diagState.enabled && !isDebug) return;
    const ts = nowMs();
...
export async function init(ctx) {
  ...
  if (__wired) {
    log('[patch_2025-09-27_phase6_polish_telemetry.init] already wired');
...
ensureCRM();
window.CRM.modules['patch_2025-09-27_phase6_polish_telemetry'] = window.CRM.modules['patch_2025-09-27_phase6_polish_telemetry'] || {};
window.CRM.modules['patch_2025-09-27_phase6_polish_telemetry'].init = init;

  ...
  window.withRender = function (lockKey, fn) {
    return new Promise((resolve, reject) => {
      const runner = () => {
        ...
        (async () => {
          try {
            // Fix: Race against 500ms timeout to prevent permanent lock
            const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error('render_timeout')), 500));
            const task = typeof fn === 'function' ? fn() : undefined;
            ...
            if (err && err.message === 'render_timeout') {
              console.warn('[render] timeout forced release', lockKey);
              // Resolve anyway so the app proceeds? Or reject?
              // Rejecting might kill the caller chain. Resolving undefined is safer for "recovery".
              resolve();
            } else {
              reject(err);
            }
...
export async function init(ctx) {
  ...
  if (__wired) {
    log('[patch_2025-09-26_phase4_polish_regression.init] already wired');
...
ensureCRM();
window.CRM.modules['patch_2025-09-26_phase4_polish_regression'] = window.CRM.modules['patch_2025-09-26_phase4_polish_regression'] || {};
window.CRM.modules['patch_2025-09-26_phase4_polish_regression'].init = init;

...
(function () {
  ...
  function wireListEvents() {
    ...
    host.addEventListener('click', async (e) => {
      ...
      // Stop propagation so we don't trigger row selection or other card-level clicks
      e.stopPropagation();
      ...
      if (btn.matches('[data-act="preview"]')) {
        const body = item.channel === 'sms' ? item.smsBody : `${item.subject}\n\n${item.emailBody}`;
        alert(body || '(empty message)');
        return;
      }
      if (btn.matches('[data-act="snooze-1"]') || btn.matches('[data-act="snooze-3"]') || btn.matches('[data-act="snooze-7"]')) {
        const days = btn.matches('[data-act="snooze-1"]') ? 1 : (btn.matches('[data-act="snooze-3"]') ? 3 : 7);
        await recordSnooze(item.contactId, item.type, days);
        toast(`Snoozed ${TYPE_LABELS[item.type] || item.type} for ${days} day(s).`);
        await renderNotifications();
        return;
      }
      if (btn.matches('[data-act="send"]')) {
        await recordSent(item.contactId, item.type, item.channel);
        try {
          await dbPut('activity', {
            id: crypto.randomUUID(),
